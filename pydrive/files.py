...
class GoogleDriveFileList(ApiResourceList):
    ...
    def _GetList(self):
        ...
        for file_metadata in self.metadata['items']:
            tmp_file = GoogleDriveFile(auth=self.auth,
...
class GoogleDriveFile(ApiAttributeMixin, ApiResource):
    ...
    def FetchMetadata(self):
        ...
        if file_id:
            try:
                metadata = self.auth.service.files()\
                    .get(fileId=file_id).execute()
    ...
    def FetchContent(self):
        ...
        if download_url:
            ...
            raise FileNotDownloadableError('No downloadLink found in metadata')
    ...
    def _BuildMediaBody(self):
        """Build MediaIoBaseUpload to get prepared to upload
           content of the file.

        Sets mimeType as 'application/octet-stream' if not specified.

        :returns: MediaIoBaseUpload -- instance that will be used
                  to upload content.
        """
        if self.get('mimeType') is None:
